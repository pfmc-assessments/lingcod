...
plot_sel_comm <- function(mod) {
  ...
  plot_sel_ret(mod, Factor = "Lsel", fleet=1)
  mtext("Selectivity", side = 2, line = 3, las = 0)
  plot_sel_ret(mod, Factor = "Lsel", fleet=2)
  plot_sel_ret(mod, Factor = "Ret", fleet=1)
  mtext("Retention", side = 2, line = 3, las = 0)
  plot_sel_ret(mod, Factor = "Ret", fleet=2)
...
#' @export
#' @author Ian G. Taylor
plot_sel_noncomm <- function(mod, area) {
  filename <- "selectivity_noncomm.png"
  filepath <- file.path(mod$inputs$dir, "custom_plots", filename)
  if (area == "n") {
    png(filepath, width = 6.5, height = 6.5, units = "in", res = 300, pointsize = 10)
    par(mfrow = c(2,2), oma = c(2,2,0,0), las = 1)
    plot_sel_ret(mod, Factor = "Lsel", fleet=3)
    plot_sel_ret(mod, Factor = "Lsel", fleet=4)
    plot_sel_ret(mod, Factor = "Lsel", fleet=5)
    fleets <- c(6,7,9)
