...
```{r figure-agelength-wcgbts, fig.height = 9, fig.retina = 3, cache = TRUE}
...
```{r figure-agelength-triennial, fig.height = 9, fig.retina = 3, cache = TRUE}
...
.pull-right[
```{r figure-agelength-lam, fig.height = 18, fig.retina = 3, cache = TRUE}
ggplot2::ggplot(bio.HKLage.Lam %>% dplyr::filter(!is.na(sex)),
  ...
  ggplot2::guides(fill = ggplot2::guide_legend(title = "Aged")) +
  ggplot2::theme(
    text = ggplot2::element_text(size=50),
    strip.background = ggplot2::element_rect(colour = "black", fill = "white"),
    legend.position = "top"
  ) +
  ggplot2::xlab("Length (cm)") +
