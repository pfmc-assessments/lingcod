...
#### California \glsentrylong{mrfss} dockside \glsentrylong{cpfv} index

From 1980-1989 and 1993-2003 the \gls{mrfss} program conducted dockside intercept surveys of the recreational \gls{cpfv} fishing fleet.
Data from `r knitr::combine_words(1990:1992)` are not available because funding for the surveys stopped.
For purposes of this assessment, the \gls{mrfss} time series was truncated at 1999 because
the onboard observer program started, which samples the same catch onboard the vessel as the dockside interviewers do on shore.
...
San Diego counties compose a third region ("S"). For models that are exclusively south 
...
the level of a fisher’s bag. Therefore, the \gls{calpoly} data are incorporated in 
...
```{asis, eval = params$area == "South", echo = TRUE, results = "asis"}
