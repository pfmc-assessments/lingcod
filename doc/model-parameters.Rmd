...
```{asis, eval = ifelse(params$area == "North", TRUE, FALSE)}
...
```{asis, eval = !any(grepl("sigma", estimated_pars))}
...
```
```{asis, eval = !any(grepl("Mat", estimated_pars))}
...
```
```{asis, eval = !any(grepl("Wtlen", estimated_pars))}
The weight-length parameters were fixed at values estimated externally using data
with paired observations of length and weight from the \gls{s-wcgbts}
