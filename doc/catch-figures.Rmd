...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-01CA-proportionnorth.png"),
  caption = paste0(
...
gg <- catch.pacfin %>%
  ...
  ggplot2::scale_fill_manual(values = unikn::usecol(pal_unikn_pair, 16L)[c(1,9)]) +
  ...
  ggplot2::xlab("Year") + ggplot2::ylab("Percentage CA commercial catch by area")
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-02CA-gearprop.png"),
  caption = paste0(
    "Percentage of California commercial catch by area within each fleet ",
    "(fixed gear, FG, in upper panel and trawl, TW, in lower panel)",
    "since 1981. ",
    "Blue represents northern area and red represents southern area."
  ),
  alttext = paste(
    ""
  )
...
gg <- ggplot2::ggplot(catch_comm_CA_interpolate,
  ...
  ggplot2::scale_colour_manual(values = unikn::usecol(pal_unikn_pair, 16L)[c(1,1,9,9)]) +
  ggplot2::theme_bw()
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-03CA-interpolate-ts.png"),
  caption = paste0(
...
gg <- ggplot2::ggplot(
  ...
  ggplot2::scale_colour_manual(values = unlist(t(get_fleet(value="Comm")[,c("col.n","col.s")]),use.names=FALSE)) +
  ggplot2::labs(
...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-04reconstruction.png"),
  caption = paste0(
    "Reconstructions of commercial landings (mt) by year (x axis), area, and fleet, ",
    "with darker colors representing catch from the northern area and ",
    "lighter colors from the southern area."
  ),
  alttext = paste(
    "Reconstructions starting in the 1800s show an increasing trend for trawl and more constant values for fixed gear."
  )
)
...
gg <- ggplot2::ggplot(
  ...
  ggplot2::geom_line(size = 1.2) +
  ggplot2::scale_colour_manual(values = unlist(t(get_fleet(value="Comm")[,c("col.n","col.s")]),use.names=FALSE)) +
  ggplot2::labs(colour = ggplot2::guide_legend(title = "area x fleet")) +
  ggplot2::theme_bw()
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-05ts-full.png"),
  caption = paste0(
...
gg <- ggplot2::ggplot(data = catch.pacfin %>%
...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-06ts-pacfin.png"),
  caption = paste0(
...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-comm-07state.png"),
  caption = paste0(
    ...
    "are show for both the northern (top panel; Oregon and Washington) and ",
...
gg <- bio_rec_recfin %>%
  ...
  ggplot2::labs(col = "recreational fleet", lty = "recreational fleet") +
  ggplot2::theme_bw() +
  ggplot2::theme(legend.position = c(0.15, 0.13)) +
  ggplot2::facet_grid(tolower(IS_RETAINED) ~ factor(sex, labels = c("female", "male", "unknown sex")))
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-rec-01tsrecfinmeanlength.png"),
  caption = paste0(
...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-rec-02CA-ts.png"),
  caption = paste0(
    "Time series of California recreational landings (mt) for the ",
    "north (blue) and south (red) areas. ",
...
gg <- ggplot2::ggplot(
  ...
  ggplot2::geom_line(ggplot2::aes(col = state)) +
  ggplot2::scale_color_manual(
...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-rec-03ca-oldts.png"),
  caption = paste0(
    "Comparison of recreational landings from this assessment (solid line) versus ",
    "the previous assessment (dashed line) for each state (colors)."
  ),
  alttext = paste(
    ""
  )
...
ggsave2(
  gg,
  filename = file.path("..", "figures", "catch-rec-04ts.png"),
  caption = paste0(
