...
split at \CapeM based on the results of a genetic analysis [@longo2020strong].
\CapeM is also the boundary used for the management of commercial catches.
...
`r ifelse(params$area == "South",
  "The final model included ages from just the \\gls{s-wcgbt} because of conflicts between age- and length-composition data.",
  ""
)`
...
uncertainty in the fraction of recruitment obtained when the population is at 20\% of unfished levels.
...
Whereas, the target biomass is in line with the SPR target (green)
(Figure \@ref(fig:es-yield)).
```
```{asis, opts.label = "south"}
The estimate of 2021 spawning biomass relative to unfished equilibrium biomass
