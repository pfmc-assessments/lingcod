...
Length data preparation followed the same methods as applied to the \gls{s-wcgbt} data,
but depth strata of 55 - 183 m and 183 - 350 m were used for expanding the
...
Figure \ref{fig:tri-lenComp} shows the length-composition distributions for the \gls{s-tri}. 
...
In collaboration with the \gls{nwfsc} and Moss Landing Marine Labs, `r spp` in nearshore 
and offshore rocky reef habitats were collected between January 2016 and January 
2017 via hook and line on chartered \glspl{cpfv} (@lam2019geographic). Sixteen latitudinal distinct sampling 
...
methods identical to those used by the onboard recreational `r spp` fishery 
...
explicit regions. Of the total fish samples, 32 were removed because they
were sampled on \gls{s-ccfrp} would have led to double counting.
...
of `r spp`. 
Figure \ref{fig:lam-lenComp} shows the length-composition distributions. 
...
(male and female) and year
(Figures \ref{fig:comp-condAALdat-bubflt7mkt0-page1}-\ref{fig:comp-condAALdat-bubflt7mkt0-page2}), and therefore,
were not expanded. The numbers of fish 
...
to being able to estimate the basic growth parameters inside 
...
The CAAL approach was only applied for male and female `r spp` and
no sex ratio was applied to unsexed fish; so, unsexed fish were excluded
from CAAL data. 
...
approach were explored by replacing CAAL compositions with marginal age compositions.

#### \acrlong{s-tri} ages

Age data preparation follow the same methods as the \gls{s-wcgbt} length data. 
The \gls{s-tri} age-composition data were included in the model 
as CAAL, and marginal age compositions were explored as sensitivities.
Figure \ref{fig:tri-ageComp} shows the age-composition data for the \gls{s-tri}.
...
was summarized using age bins from 0 to 20 years, in increments of one year. The last bin 
includes all fished aged to be greater than 20 years.
The Hook and Line Survey age-composition data were included in the model as CAAL,
and marginal age compositions were explored as sensitivities.
Figure \ref{fig:hkl-ageComp} shows the age-composition data for the Hook and Line Survey.
Table \ref{tab:sample-size-age} shows sample sizes.
```
...
Figure \ref{fig:lam-ageComp} shows the age-composition distributions. 
Table \ref{tab:sample-size-age} shows sample sizes.
