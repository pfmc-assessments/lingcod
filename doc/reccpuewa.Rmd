...
`sprintf("%.2f", max(reccpuewa_filtersummary[["% pos."]]))`
...
The Gamma distribution was chosen going forward given match of the theoretical quantiles to the
data quantiles (Figure \@ref(fig:)).

Based on the the negative binomial model were reasonable (Figures \@ref(fig:fig-posterior-mean-ORAtSea) and \@ref(fig:fig-posterior-sd-ORAtSea)). The negative binomial model generated data sets with the proportion zeros similar to the `r scales::percent(1-dim(dat %>% filter(Target>0))[1]/dim(dat)[1])` zeroes in the observed data 
...
```{r recwacpue-readfigures, results="asis", echo = FALSE}
