...
```
```{r catch-readpacfin}
# Automated but needed for other things later
# Read in PacFIN file
...
<!--- COMMERCIAL INFORMATION STARTS HERE -->
## Commercial landings
...
```
```{asis, catch-comm-WA-text, opts.label = 'north'}
...
```{r catch-comm-OR, eval = TRUE, include = FALSE}
...
```
```{asis, catch-comm-OR-reconstruction, opts.label = 'north'}
...
region from block data [@Miller_2014]
...
```
```{asis, catch-rec-WA-text, opts.label = 'north'}
...
```
```{asis, catch-rec-OR-text, opts.label = 'north'}
#### Oregon
...
```{asis catch-tables-title, eval = ifelse(knitr::opts_knit$get("child"), FALSE, TRUE)}
### Tables
```
```{r catch-tables-parent, eval = ifelse(knitr::opts_knit$get("child"), FALSE, TRUE), child = "catch-tables.Rmd"}
```
```{asis catch-figures-title, eval = ifelse(knitr::opts_knit$get("child"), FALSE, TRUE)}
### Figures
```
```{r catch-figures-parent, eval = TRUE, child = "catch-figures.Rmd", include = FALSE}
```
