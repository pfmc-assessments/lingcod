...
```
```{r catch-readpacfin, include = FALSE}
...
```{asis, catch-comm-OR-recent, opts.label = 'north'}
Though `r utils_name("common")` are encountered on a wide variety of gear types in Oregon,
...
catch_rec_WA <- suppressWarnings(suppressMessages(
  ...
  dplyr::summarize(.groups = "keep",
    num = sum(LINGCOD)
    # Below if using total discard + retained
...
catch_rec_OR <- dplyr::full_join(by = c("Year", "mt"), .id = "dataset",
  ...
  readxl::read_excel(
    ...
    dplyr::select(!dplyr::matches("MORTALITY_MT|REL")) %>%
...
2009 assessment of `r utils_name("common")` [@hamel2009].
...
\gls{mrfss} data are not available for
...
and is still currently being collected.
Information includes on port group that was used to partition landings
to the northern and southern areas.
