...
through
`r sprintf("%.0f.", max(catch_comm_WA[["Year"]]))`
...
```{r catch.pacfin, eval = TRUE, include = FALSE}
...
```
```{r catch_comm_OR, eval = TRUE, include = FALSE}
...
`r min(catch_comm_OR[["Year"]]) `
to
`r paste0(max(catch_comm_OR[["Year"]]), ".") `
...
`r NROW(catch_sette1928 %>% dplyr::filter(lbs > 0)) - 1`
years.
...
`r knitcitations::citep("10.1371/journal.pone.0099758")`
...
```{r catch-comm-CA-reconstructionprops, fig.cap = "Calculated proportion of California (CA) commercial catch landed north of forty degrees ten minutes latitude compared to that landed south from two data sources, the Raltson et al. (2010) catch reconstruction effort (open circles) and fish ticket data in CALCOM (filled circles)."}
...
the following years,
`r paste0(knitr::combine_words(catch_comm_CA_MM[catch_comm_CA_MM[["type"]] == "ALL","Year"]), ",") `
...
to
`r catch_sette1928 %>% dplyr::filter(mt > 0) %>% dplyr::pull(mt) %>% dplyr::first()`
mt in
...
```
```{r catch-comm-CA-interpolate-ts, fig.width = 15, fig.cap = "Reconstructed commercial landings for the state of California by fleet (fixed gear, FG; trawl gear, TW) and area. Dashed line indicates when the data were linearly interpolated versus inferred from references."}
...
`r sprintf("%s.", min(catch.pacfin[["Year"]]))`
...
```{r catch-comm-CA-gearprop, fig.width = 15, fig.cap = "Percentage of California catch by area within each fleet since 1981."}
catch.pacfin %>%
...
```{r catch-comm-seasonality, fig.cap = "Seasonality (month; x axis) of landings (mt) by area (column) and fleet (row). The y axis is on the log scale, colors represent years, and circles are transparent to facilitate visualization of months with many records."}
...
```
```{r catch-comm-ts-pacfin, fig.width = 8, fig.cap = "Yearly commercial landings (mt) from the PacFIN database since 1981 by area (panel and color) and fleet (shading). Trawl gear (TW) includes all trawl, nets, and dredging. Fixed gear (FG) includes all other gear types."}
ggplot2::ggplot(data = catch.pacfin %>%
...
```
```{r data-catch-comm-reconstruction, fig.cap = "Reconstructions of commercial landings (mt) by year, area, and fleet."}
ggplot2::ggplot(
...
```
```{r data-catch-comm, fig.cap = "Commercial landings (mt) by year, area, and fleet."}
ggplot2::ggplot(
  catch_comm,
  ggplot2::aes(Year, mt, col = interaction(fleet, area, sep = " "))
) +
...
**todo: get a written description of methods from aliwhitman**
...
model as its own fleet rather than combining with Oregon recreational data
...
```
```{r investigatediffinORRec, include = FALSE, echo = FALSE, eval = FALSE}
dplyr::full_join(
  by = c(Year = "year"),
  x = catch_rec %>%
    dplyr::group_by(Year) %>%
...
```{r catch-rec-ts, fig.cap = "Time series of recreational landings by state and area."}
ggplot2::ggplot(
  catch_rec,
