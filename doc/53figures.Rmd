...
```
```{asis, opts.label="north", eval=FALSE}
...
```{asis, cpue-figures-south, opts.label = "south", eval=FALSE}
...
get_fleet("rec") %>%
  ...
  dplyr::filter(!!as.name(paste0("used_", "2021",".", tolower(substr(params$area,1,1))))) %>%
...
<!-- ====================================================================== -->
```{r ORatseacpuefigures, results = "asis", eval = FALSE}
# note, `eval = params$area == "North"` changed to `eval = FALSE`
sa4ss::add_figure(
...
<!-- Hook and line -->
```{r hookandlinesurveyfigures, results = "asis", eval = FALSE}
# note, `eval = params$area == "South"` changed to `eval = FALSE`
sa4ss::add_figure(
...
<!-- s-ccfrp figures -->
```{asis, opts.label = 'south', echo = TRUE, eval=FALSE}
