...
```{r, cpue-figures-north, results = "asis", opts.label = "north"}
...
ignore <- apply(
  figures_cpuedata %>% dplyr::filter(grepl("cpue-commercialfixed", label)),
  1,
  function(x) do.call(sa4ss::add_figure, args = as.list(x))
)
ignore <- apply(
  utils::read.csv(file.path("..", "figures", "reccpuewa.csv")),
  1,
  function(x) do.call(sa4ss::add_figure, args = as.list(x))
)
ignore <- apply(
  figures_cpuedata %>% dplyr::filter(grepl("cpue-recreationalOregon", label)),
  1,
  function(x) do.call(sa4ss::add_figure, args = as.list(x))
)
...
```{r, results = "asis"}
get_fleet("Surv|Res") %>% dplyr::arrange(order) %>%
...
ignore <- mapply(FUN = sa4ss::add_figure,
  ...
  caption = paste0(c("Length", "Age"), " frequency for males and females in the \\glsentrylong{wcgbts} (\\glsentryshort{wcgbts})."),
...
```{r, results = "asis", opts.label = "north"}
...
do.call(
  sa4ss::add_figure,
  utils::read.csv(file.path("..", "figures", "ageing-double-reads.csv"))
)
```
