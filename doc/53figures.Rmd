...
figurecsv <- utils::read.csv(
  file.path("..", "models", params$model, "plots", "plotinfotable_for_doc.csv")
)
```
...
do.call(
  sa4ss::add_figure,
  utils::read.csv(file.path("..", "models", params$model,  "index_data_all_fleets.csv"))
)
```
...
mapply(
  ...
  SIMPLIFY = FALSE,
  file.path("..", "data-raw",
    sprintf("%s_%s_gamma", c("WCGBTS", "Triennial"), "North")
    # sprintf("%s_%s_gamma", c("WCGBTS", "Triennial"), params$area)
  )
) %>% dplyr::bind_rows(.id = "source") %>% add_figure_ling
```
...
do.call(
  sa4ss::add_figure,
  utils::read.csv(file.path("..", "models", params$model,  "index_fits_all_fleets.csv"))
)
```
